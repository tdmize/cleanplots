...
replicates the non-color settings of the Stata scheme: white background,
no plot-region border, light gray axis lines and ticks, dotted light
gray gridlines in both directions, and a frameless, untitled legend at
the lower right. To restore a legend title:
...
to get the same behavior in ggplot2:
...
The palette alternates dark and light colors so the first several series
...
of about 21). Beyond 4-5 series, add
[`scale_shape_cleanplots()`](https://tdmize.github.io/cleanplots/reference/scale_shape_cleanplots.md)
(points) or
[`scale_linetype_cleanplots()`](https://tdmize.github.io/cleanplots/reference/scale_shape_cleanplots.md)
(lines): shapes and patterns are assigned so that any two series sharing
a shape or pattern always differ strongly in lightness. Check the
palette yourself at [Coloring for
